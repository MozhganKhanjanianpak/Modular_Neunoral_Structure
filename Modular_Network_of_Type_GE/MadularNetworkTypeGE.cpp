...
#define p_in_I  0.8  // connection probability inside blocks
#define p_in_E  0.6  // connection probability inside blocks
#define p_out_E 0.029 // connection probability outside blocks
 		...
 		for (int j=0 ; j<N ; j++){
			...
			int start=sumN+Eper*newN;
 			if ( j>=sumN && j<sumN+newN ){
 				if (r<p_in_E && i<start)  A[i][j]=Wexcitatory;
 				else if (r<p_in_I && i>=start)  A[i][j]= Winhibitory;
			}
			else{
 				if (r<p_out_E && i<start)
 					A[i][j]=Wexcitatory;				
			} 
		}
	}
